...
if True:
    ...
    def connect():
        ...
        emit('init_msg', {'user': 'System', 'msg': msg, 'userid':session['user']}, broadcast=False)
    ...
    def c_send_msg(data):  # 요기서 댓글 텍스트 처리
        ...
        if filter_need:
            ...
            emit('s_send_msg', {'user': session['user'], 'msg': msg}, broadcast=True) 
        else:
            emit('s_send_msg', {'user': session['user'], 'msg': output_text}, broadcast=True)
...
def chat():
    ...
    print(ip_addr)
    return render_template('index.html', keywords = filtering_keyword,ip_addr=ip_addr)
